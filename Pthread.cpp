...
{
	threadParam_bitmap* p = (threadParam_bitmap*)param_bit;
	int isfound = p->isfound;
	int id = p->t_id;
	for (unsigned int j = 0; j < bm[0].second_index.size(); j++)
	{
...
void* threadFunc_search_element_d(void* param)
{
	...
	for (int m = p->n_idx.len; m < length; m++)
	{
		p->n_idx.order[m] = 0;
	}
	...
	if (p->judge == true)
	{
		...
		for (int i = 0; i < p->n_idx.len; i += 4) {
			__m128i var1, var;
			var1 = _mm_loadu_epi32(&p->n_idx.order[i]);
			var = _mm_set1_epi32(0);
			var = _mm_sub_epi32(var0, var1);
			int compare[4];
			_mm_storeu_epi32(compare, var); // 只存储一次比较结果  
			for (int j = 0; j < 4; j++) {
				if (compare[j] == 0) {
					found = true; // 直接更新外部的found变量  
					break;
				}
			}
			if (found) break; // 如果找到，则退出循环  
		}
		p->isfound = found; // 更新结构体中的搜索结果标志  
	}
}
		...
		for (j = 1; j <= num;)
		{
			for (int o = 0; o < 4; o++, j++)
			{
				if (j < num)
				{
					...
					param[o].judge = true;
					pthread_create(&thread[o], NULL, threadFunc_search_element_d, &param[o]);
				}
				else
				{
...
void* threadFunc_search_element_s(void* param)
{
	...
	{
		int s = 1;
		unsigned int var = idx0[0].order[0];
		while (s != p->num_of_query && found == true)
		{
		...
		if (s == p->num_of_query && found == true)
		{
			n_tmp[p->t_id].push_back(s);
		}
	}
	sem_post(&sem_main);
	return p;
...
void search_element_SIMD_SSE_P_s(int* query, vector<index>& idx, int num)
{
	...
	for (int t_id = 0; t_id < NUM_THREADS; t_id++)
	{
		...
		param[t_id].num_of_query = sizeof(query);
		param[t_id].num_of_query = num;
...
void search_list_bit_SIMD_SSE_P(int* query, vector<index>& idx, int num)
{
	...
	int main()
	{
		...
		outfile.close();
		cout << "search_list_bit_SIMD_SSE:";
		gettime(search_list_bit_SIMD_SSE_P, t_query, idx);
		return 0;
	}
